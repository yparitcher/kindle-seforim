...
# directories
#####
startpoint="Steinsaltz_talmud_Hebrew"
...
$(node $scripts/talmud.js "$source/index.json")
EOF

if [[ ! -d "$intermediate/biur" ]]; then mkdir -p "$intermediate/biur"; fi;

counter=1
while read i; do
	dest="$(printf '%02d' $counter)_$i"
	dest="${dest//Steinsaltz on /}"
	dest="${dest// /_}"
	if [[ ! -f "$intermediate/biur/$dest.json" ]]; then
		wget -O "$intermediate/biur/$dest.json" "https://www.sefaria.org/api/texts/$i?pad=0";
	fi;
	counter=$(($counter+1));
done << EOF
$(node $scripts/biur.js "$source/index.json")
EOF

for i in $intermediate/*.json; do
	biur="$intermediate/biur/$(basename $i)"
	if [ -f "$biur" ]; then
		dest="$(basename --suffix .json $i)"
		node "$scripts/$startpoint.js" "$i" "$biur" "../../$source/gematriya.js" > "$intermediate/$dest.html"
	fi
done
