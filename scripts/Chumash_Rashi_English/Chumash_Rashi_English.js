		...
		for (let i=0; i<jsonData.he[x].length; i++) {
			if (jsonData.he[x][i] != "") {
				if (jsonData.alts[x] != undefined && jsonData.alts[x][i] != undefined && jsonData.alts[x][i].whole ){
					...
					fs.writeFileSync(file, '<title class="title">' + jsonData.alts[x][i]["he"][0] + "</title>\n");
					fs.appendFileSync(file, '<h2 class="h2">' + jsonData.alts[x][i]["he"][0] + "</h2>\n");
					fs.appendFileSync(file, '<h3 class="h3">' + "פרק " + gematria(x+1) + "</h3>\n");
				} else if (jsonData.alts[x] != undefined && jsonData.alts[x][i] != undefined){
					fs.appendFileSync(file, '<h2 class="h2">' + jsonData.alts[x][i]["he"][0] + "</h2>\n");
				} else if (i == 0){
					fs.appendFileSync(file, '<h3 class="h3">' + "פרק " + gematria(x+1) + "</h3>\n");
				}
				fs.appendFileSync(file, '<p class="right"><b>' + gematria(i+1) + " </b>" + jsonData.he[x][i] + "</p>\n");
				if (jsonData.text[x] != "" && jsonData.text[x][i] != undefined) {
					fs.appendFileSync(file, '<p class="left">' + jsonData.text[x][i] + "</p>\n");
				}
				if (commentaryData.he[x]!= undefined && commentaryData.he[x][i] != 0 && commentaryData.he[x][i] != undefined ) {
					for (let y=0; y<commentaryData.he[x][i].length ; y++) {
						fs.appendFileSync(file, '<p class="right">' + commentaryData.he[x][i][y] + "</p>\n");
						if (commentaryData.text[x] != undefined && commentaryData.text[x][i] != undefined && commentaryData.text[x][i][y] != undefined) {
							fs.appendFileSync(file, '<p class="left">' + commentaryData.text[x][i][y] + "</p>\n");
						}
					}
				}
			}
